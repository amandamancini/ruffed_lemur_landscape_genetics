##########################################################################################################################################
################# Combination B Multi Surface Redo Optimization 1 #######################################################################################
##########################################################################################################################################
...
dir.create(file.path(paste(getwd(), "/Results/Multi_Unrelated/", sep=""), "MS_Unrelated_Combo_B_Results_Opt1"))

### STEP 7: Paste .asc files into the newly created sub-directory

combo.b <- c("DSM_final.asc", "TPI_final.asc")
ss.results.opt1 <- paste(getwd(), "/Results/Multi_Unrelated/MS_Unrelated_Combo_B_Results_Opt1/", sep="")

file.copy(file.path(source.folder, combo.b), ss.results.opt1)

### STEP 8: Set directory to write results of first optimization

write.dir.opt1 <- paste(getwd(), "/Results/Multi_Unrelated/MS_Unrelated_Combo_B_Results_Opt1/", sep="")

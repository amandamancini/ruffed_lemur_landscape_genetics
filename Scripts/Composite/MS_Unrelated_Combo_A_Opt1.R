##########################################################################################################################################
################# Combination A Multi Surface Redo Optimization 1 #######################################################################################
##########################################################################################################################################
...
dir.create(file.path(paste(getwd(), "/Results/Multi_Unrelated/", sep=""), "MS_Unrelated_Combo_A_Results_Opt1"))

### STEP 7: Paste .asc files into the newly created sub-directory

combo.a <- c("DSM_final.asc", "TPI_final.asc", "TRI_final.asc")
ss.results.opt1 <- paste(getwd(), "/Results/Multi_Unrelated/MS_Unrelated_Combo_A_Results_Opt1/", sep="")

file.copy(file.path(source.folder, combo.a), ss.results.opt1)

### STEP 8: Set directory to write results of first optimization

write.dir.opt1 <- paste(getwd(), "/Results/Multi_Unrelated/MS_Unrelated_Combo_A_Results_Opt1/", sep="")
...
GA.inputs <- GA.prep(method = "LL",
                     ...
                     max.cat = 10000,
                     max.cont = 10000,
                     select.trans = list('A', 'A', 'A'), #DSM, TPI, TRI
                     parallel = 32) 

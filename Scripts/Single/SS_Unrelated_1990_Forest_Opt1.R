##########################################################################################################################################
################# 1990 Forest Single Surface Optimization 1 #######################################################################################
##########################################################################################################################################
...
dir.create(file.path(paste(getwd(), "/Results/Single_Unrelated/", sep=""), "SS_Unrelated_1990_Forest_Results_Opt1"))
...
forest.1990 <- c("1990_forest_final.asc")
ss.results.opt1 <- paste(getwd(), "/Results/Single_Unrelated/SS_Unrelated_1990_Forest_Results_Opt1/", sep="")

file.copy(file.path(source.folder, forest.1990), ss.results.opt1)

### STEP 8: Set directory to write results of first optimization

write.dir.opt1 <- paste(getwd(), "/Results/Single_Unrelated/SS_Unrelated_1990_Forest_Results_Opt1/", sep="")
